...
- `tensorize_tt` still depends on circular buffer heuristics (`_tile` fallback) and does not yet export a fully deterministic `T.gemm` lowering path.
...
| Task ID | Description |
|---------|-------------|
| `refactor-tensorize-tt` | **Refactor `tensorize_tt.cc` to Prioritize `T.gemm` Lowering**: Separate explicit `T.gemm` lowering from heuristic detection and build a robust, well-tested path. |
