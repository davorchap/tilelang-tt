...
- Heuristic matmul pattern detection in `tensorize_tt`; the consolidation prioritizes explicit `T.gemm` lowering.
- Supporting multiple matmuls per reduction loop inside `tensorize_tt`; this extension follows after the primary path is stable.
...
- `tensorize_tt` now targets explicit `T.gemm` regions; it still falls back to `_tile` CB heuristics until `bug-fix-cb-id-resolution` lands.
...
| Task ID | Description |
|---------|-------------|
| `refactor-tensorize-tt` | ✅ **Refactor `tensorize_tt.cc` to Prioritize `T.gemm` Lowering**: Explicit GEMM markers drive tensorization; next iteration will consume `tl.gemm` intrinsics directly and drop the residual matcher. |
| `bug-fix-cb-id-resolution` | **Remove CB ID Heuristics**: Delete `_tile` suffix fallbacks so `tensorize_tt` relies strictly on `tt_circular_buffers` metadata. |
...
*Feedback highlights*:
- Ensure all runtime metadata emitted by `tensorize_tt` is canonical so compute codegen remains a straightforward printer.
