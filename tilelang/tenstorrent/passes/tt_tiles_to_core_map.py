...
from ..attrs import (
    TT_CORE_GRID, TT_CORE_RANGES, TT_CORE_RANGE, TT_WORK_PARTITION,
    TT_LAYOUT_DESC, CoreRange, WorkItem
)
            ...
            for param in func.params:
                ...
                if buffer and buffer.name in ["C", "output"]:
                    ...
                    if len(shape) >= 2:
                        ...
                        logger.debug(f"Inferred tile counts from buffer {buffer.name}: "
                                   f"M={tile_info['output_tiles_m']}, N={tile_info['output_tiles_n']}")
                        break
        
        return tile_info
    
    def _create_work_partition(self, 
                              grid: Tuple[int, int],
